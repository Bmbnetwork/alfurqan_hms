...
$vitals_today = $conn->query("SELECT COUNT(*) as total FROM vitals WHERE DATE(recorded_at) = CURDATE()")->fetch_assoc()['total'] ?? 0;
$critical_patients = $conn->query("SELECT COUNT(*) as total FROM patients WHERE patient_status = 'Critical'")->fetch_assoc()['total'] ?? 0;

// Recent Vitals
$recent_vitals = $conn->query("SELECT v.*, p.name as patient_name, u.username as nurse_name 
                               FROM vitals v 
...
<head>
    <meta charset="UTF-8">
    <title>Nurse Dashboard | Alfurqan Clinic</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .section h3 { color: #004d40; margin-bottom: 20px; border-bottom: 2px solid #e0f2f1; padding-bottom: 15px; }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card"><h3>Vitals Recorded Today</h3><p><?php echo $vitals_today; ?></p></div>
            <div class="stat-card critical"><h3>Critical Patients</h3><p><?php echo $critical_patients; ?></p></div>
        </div>
        
        <div class="section">
            <h3>️ Recently Recorded Vitals</h3>
            <?php if ($recent_vitals->num_rows > 0): ?>
            <table>
                <thead><tr><th>Date/Time</th><th>Patient</th><th>Temp (°C)</th><th>BP</th><th>Pulse</th><th>SpO2</th><th>Recorded By</th></tr></thead>
                <tbody>
                    <?php while($v = $recent_vitals->fetch_assoc()): ?>
                    <tr>
                        <td><?php echo date('d M Y, g:i A', strtotime($v['recorded_at'])); ?></td>
                        <td><?php echo htmlspecialchars($v['patient_name']); ?></td>
                        <td><?php echo $v['temperature']; ?></td>
                        <td><?php echo $v['blood_pressure']; ?></td>
                        <td><?php echo $v['pulse_rate']; ?></td>
                        <td><?php echo $v['oxygen_saturation']; ?>%</td>
            ...
            <?php else: ?>
            <p style="text-align: center; color: #666; padding: 20px;">No vitals recorded yet.</p>
            <?php endif; ?>
        </div>
    </div>
</body>
